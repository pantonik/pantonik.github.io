...
Although the performance of these hardware implementations is comparable to state-of-the-art digital algorithms on a series of benchmark tasks, there are some bottlenecks. One of them is the **readout layer**, commonly consisting of a slow offline post-processing. Few **analogue** solutions have been proposed (in particular by the [OPERA-Photonics](https://www2.ulb.ac.be/facs/polytech/opera/index.html) group at ULB), but all suffered from noticeable decrease in performance due to added complexity of the setup.
